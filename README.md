...
| `search_tools` | Поиск групп и тулов по имени: первое слово запроса ищется (без учёта регистра) как подстрока в имени группы и в именах вложенных тулов. В ответе — совпавшая группа и **только** совпавшие тулы внутри неё, а не вся группа |
...
`search_tools` — целевой поиск вместо полного `list_tools`: LLM передаёт
свободный `query`, расширение берёт **первое слово** и ищет его (без учёта
регистра) как подстроку в имени группы и в именах тулов внутри группы
(а также в именах отдельных тулов из `individual_tools`). Формат ответа — тот
же, что у `list_tools`, но в секции `tools` группы перечисляются только
совпавшие тулы. Далее модель включает найденную группу через `enable_tool`
по имени из результата поиска.
...
| `tool_opts` | `table<string, ToolOpts>` | `{}` | Настройки отдельных тулов. Ключ — имя тула (`list_tools`, `search_tools`, `enable_tool`, `disable_tool`) |
...
Тул может быть объявлен либо фабрикой (`path`/`callback`/`extends`), либо
инлайн-определением (`schema`/`cmds`) — последним способом расширения вроде
`codecompanion-subagents.nvim` пишут тулы прямо в конфиг.
...
> **Примечание:** `list_tools` не принимает аргумент `name`, а `search_tools`
> принимает `query`, поэтому `no_approval_for` на них никак не влияет — их
