...
M.mcp_enable_server = {
  ...
  cmds = {
    function(self, args, _input)
      ...
      local chat_note
      if chat then
        chat_note = " The server's tools are now registered into this chat and become callable on the next turn."
      else
  ...
  output = {
    ...
    success = function(self, stdout, meta)
      local chat = meta.tools.chat
      local llm_output = vim.iter(stdout or {}):flatten():join("\n")
      chat:add_tool_output(self, llm_output, fmt("MCP server %s enabled", tool_display_name(self.args.name or "?")))
    end,
    error = function(self, stderr, meta)
      local chat = meta.tools.chat
      local err = vim.iter(stderr or {}):flatten():join("\n")
...
M.mcp_disable_server = {
  ...
  output = {
    ...
    success = function(self, stdout, meta)
      local chat = meta.tools.chat
      local llm_output = vim.iter(stdout or {}):flatten():join("\n")
      chat:add_tool_output(self, llm_output, fmt("MCP server %s disabled", tool_display_name(self.args.name or "?")))
    end,
    error = function(self, stderr, meta)
      local chat = meta.tools.chat
      local err = vim.iter(stderr or {}):flatten():join("\n")
